...
function normalizeSizeValue(size) {
  const value = getNumericSize(size);
...
function getAvailableSizes() {
  ...
  return [
    ...new Set(
      sneakers
        .map(item => getNumericSize(item.size))
...
function getEditionCategories(sneaker) {
  ...
  if (
    ...
    (corpus.includes("SIGNATURE") && corpus.includes("SIGNED"))
  ) {
...
function getConditionCategory(sneaker) {
  const value = getLocalizedText(sneaker.condition)
    .trim()
    .toLowerCase();
...
function filterSneakers(items) {
  return items.filter(sneaker => {
    if (activeFilters.edition.size > 0) {
      const categories = getEditionCategories(sneaker);
      const match = [...activeFilters.edition]
        .some(value => categories.has(value));

      if (!match) return false;
    }

    if (activeFilters.condition.size > 0) {
      if (!activeFilters.condition.has(getConditionCategory(sneaker))) {
        return false;
      }
    }
...
function getReleaseTimestamp(value) {
  ...
  const normalized = /^\d{4}-\d{2}$/.test(text)
    ? `${text}-01`
    : text;
...
function sortSneakers(items) {
  ...
  switch (currentSort) {
    case "az":
      sorted.sort((a, b) =>
        getLocalizedText(a.title).localeCompare(
    ...
    case "za":
      sorted.sort((a, b) =>
        getLocalizedText(b.title).localeCompare(
    ...
    case "size-asc":
      sorted.sort((a, b) => {
        const aSize = getNumericSize(a.size);
        const bSize = getNumericSize(b.size);
...
function changeSort(value) {
  ...
  currentSort = allowed.includes(value) ? value : "default";
  renderGrid();
}
...
function renderSizeFilters() {
  ...
  container.innerHTML = getAvailableSizes()
    .map(size => {
...
function updateFilterInterface() {
  document
    .querySelectorAll(".filter-chip[data-group]")
    .forEach(button => {
  ...
  if (clearButton) {
    ...
    clearButton.classList.toggle("visible", hasActiveFilters);
  }
}
...
function updateStaticText() {
  ...
  const title = document.querySelector('[data-i18n="mainTitle"]');
  const subtitle = document.querySelector('[data-i18n="mainSubtitle"]');

  if (title) title.textContent = t.mainTitle;
  if (subtitle) subtitle.textContent = t.mainSubtitle;

  const sortLabel = document.getElementById("sort-label");
  const filterTitle = document.getElementById("filter-title");
  const clearButton = document.getElementById("clear-filters");
  const editionLabel = document.getElementById("edition-filter-label");
  const conditionLabel = document.getElementById("condition-filter-label");
  const sizeLabel = document.getElementById("size-filter-label");
  ...
  if (editionLabel) editionLabel.textContent = t.editionLabel;
  if (conditionLabel) conditionLabel.textContent = t.conditionLabel;
  ...
  if (select) {
    ...
    [...select.options].forEach(option => {
      option.textContent = names[option.value] || option.textContent;
    });

    select.value = currentSort;
  }

  document.getElementById("btn-vi")?.classList.toggle(
    "active",
    currentLang === "vi"
  );
...
function updateCollectionCount(count) {
  ...
  element.textContent = translations[currentLang].total(count);
}
...
function renderGrid() {
  ...
  if (sorted.length === 0) {
    grid.innerHTML = `
      <div class="not-found">
        ${escapeHTML(translations[currentLang].noResults)}
      </div>
    `;
    return;
  }
...
function setLanguage(lang) {
  ...
  localStorage.setItem("locan_lang", currentLang);
